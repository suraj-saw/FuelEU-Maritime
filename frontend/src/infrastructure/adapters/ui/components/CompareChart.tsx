// frontend/src/infrastructure/ui/components/CompareChart.tsx
...
export function CompareChart({ baseline, comparison }: Props) {
  const data = comparison.map((c) => ({
    ...
    target: TARGET,
    diff: baseline
      ? ((c.ghgIntensity / baseline.ghgIntensity - 1) * 100).toFixed(2)
      : "0",
              ...
              <tr key={d.routeId}>
                ...
                <td
                  className={`px-4 py-2 text-sm font-medium ${
                    d.compliant === "Yes" ? "text-green-600" : "text-red-600"
                  }`}
                >
